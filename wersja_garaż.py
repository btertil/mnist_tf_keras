...
bss = [32, 64, 128, 256, 512, 1024, 2048, 4096, 8192]
...
for es in ess:
    for bs in bss:
        ...
        # log automatically to mlflow (metrics + model artifact)
        # mlflow.keras.autolog()

        with mlflow.start_run(experiment_id=experiment_id) as run:

            # Not needed since model and params will be logged automatically
            mlflow.log_param("batch_size", bs)
            mlflow.log_param("epochs", es)

            # My custom metrics and artifacts to be logged into MLFlow
            ...
            mlflow.keras.log_model(model2, f"keras_model_cb_{bs}_{es}_es")
